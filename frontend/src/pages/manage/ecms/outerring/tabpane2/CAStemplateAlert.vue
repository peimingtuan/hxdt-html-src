    ...
    <transition name="slide-fade">
      <div class="alert-window2" v-show="isMain2">
        ...
        <deployAlert
          ...
          @selectshow="isMain3=false"
          @qure="updateSettingSubmit"
          @cancel="childernCancel"
        />
      </div>
    </transition>
  </div>
...
import deployAlert from "./CASdeployAlert";
import Tool from "../../js/tools";
import getApiUrl from "../../js/getApiUrl";
export default {
  name: "CAStemplateAlert",
  props: {
    isMain2: Boolean,
    selectshow: Function
  },
  ...
  methods: {
    ...
    getTableData() {
      ...
      _this.$http
        .post("/casconfig/getCASClientTemplets", {
          pageIndex,
          pageSize
        })
        .then(data => {
    ...
    download(index, row) {
      let _this = this;
          ...
          let url = window.location.origin + getApiUrl(
            "/casconfig/downloadPicture?templateId=" + templateId
          );
    ...
    // 配置弹框的确认按钮
    updateSettingSubmit() {
      this.$refs.deployAlert.submitForm();
    },
    // 删除
    delinfo(index, row) {
      let _this = this;
    ...
    childernCancel(){
      this.isMain3=false;
      this.$refs.deployAlert.backFillData()
    },
    //分页信息
    //当前页数
