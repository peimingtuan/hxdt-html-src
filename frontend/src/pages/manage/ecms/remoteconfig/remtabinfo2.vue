    ...
    <el-collapse v-model="activeName" accordion @change="openchange">
      <el-collapse-item title="庭审设备控制" name="1">
        ...
        <div class="table-style">
          ...
          >
            ...
            <el-table-column prop="courtDeviceIp" label="设备IP" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column prop="courtDeviceModelName" label="设备型号" show-overflow-tooltip></el-table-column>
            <el-table-column prop="courtDeviceSerialNumber" label="序列号" show-overflow-tooltip></el-table-column>
            <el-table-column prop="courtDeviceVersion" label="版本号" show-overflow-tooltip></el-table-column>
            <el-table-column prop="manufacturerName" label="厂商" show-overflow-tooltip></el-table-column>
      ...
      <el-collapse-item title="信号源配置" name="2">
        ...
        <div class="table-style">
          ...
          >
            <el-table-column type="selection"></el-table-column>
            <el-table-column prop="viewName" label="信号源名称" show-overflow-tooltip></el-table-column>
            <!-- <el-table-column prop="courtDeviceModelName" label="设备型号" show-overflow-tooltip></el-table-column> -->
            <el-table-column prop="signalSourceIp" label="编码器IP" show-overflow-tooltip></el-table-column>
            <el-table-column prop="channelNum" label="通道号" show-overflow-tooltip></el-table-column>
            <!--<el-table-column prop="undertakeJudge" label="输出通道类型" show-overflow-tooltip>-->
            <!--</el-table-column>-->
            <!--<el-table-column prop="resolution" label="分辨率" show-overflow-tooltip></el-table-column>-->
            <el-table-column prop="flowAddress" label="流地址" width="220px" show-overflow-tooltip></el-table-column>
      ...
      <el-collapse-item title="解码器通道配置" name="3">
        ...
        <div class="table-style">
          ...
          >
            <el-table-column prop="channelName" label="输出设备名称" show-overflow-tooltip></el-table-column>
            <el-table-column
              prop="signalSourceIp"
              label="解码器IP"
              width="150px"
...
export default {
  ...
  methods: {
    receiveTrialRoomId(trialRoomId) {
      ...
      this.activeName = "1";
      this.tableData1 = []; //法庭设备控制列表,
      this.tableData2 = []; //信号源配置列表,
      this.tableData3 = []; //解码器通道配置列表,
      this.httpinfo1(trialRoomId);
    },
    //展开面板
    openchange(v) {
      if (v == 1) {
        this.httpinfo1(this.trialRoomId);
      } else if (v == 2) {
        this.pageIndex2=1;
        this.httpinfo2(this.trialRoomId);
      } else if (v == 3) {
        this.httpinfo3(this.trialRoomId);
      }
    },
    httpinfo1() {
      let _this = this;
      _this.$http
        .post("/remoteTrial/getTrialRoomDeviceInfosByPage", {
          trialRoomId: this.trialRoomId, //法庭id
          pageIndex: this.pageIndex,
          pageSize: this.pageSize
        })
        .then(data => {
          console.log("法庭设备配置", data);
          if (data.code === 200) {
        ...
        .catch(err => {
          console.log(err, "/remoteTrial/getTrialRoomDeviceInfosByPage");
    ...
    add1() {
      ...
      this.$refs.infoAlertfirst3.receiveID(this.trialRoomId);
    ...
    delinfo1(index, row) {
      ...
      _this
        ...
        .then(() => {
          _this.$http
            .post("/remoteTrial/deleteTrialRoomDeviceInfo", {
              deviceId: row.courtDeviceId //法庭设备id
            })
            .then(data => {
              if (data.code === 200) {
                _this.$msgs("删除成功!");
                ...
                _this.httpinfo1(this.trialRoomId);
              } else {
                _this.$msgw(data.message);
              }
            })
            .catch(err => {
              console.log(err, "/remoteTrial/deleteTrialRoomDeviceInfo");
              _this.$msge("服务器异常，请稍后重试");
            });
        })
        .catch(() => {});
    ...
    httpinfo2() {
      let _this = this;
      _this.$http
        .post("/remoteTrial/getTrialRoomSignalSourceInfosByPage", {
          trialRoomId: this.trialRoomId, //法庭id
    ...
    add2() {
      //调用子组件方法
      this.$refs.infoAlertsecond3.receiveID(this.trialRoomId);
    ...
    delinfo2(index, row) {
      ...
      that
        ...
        .then(() => {
          that.$http
            .post("/remoteTrial/deleteTrialRoomSignalInfo", {
            ...
            .then(data => {
              if (data.code === 200) {
                ...
                that.pageIndex2=1;
                that.httpinfo2(this.trialRoomId);
              } else {
    ...
    setFont(index, row) {
      //调用子组件方法
      this.$refs.setinfoAlert3.show(row, this.trialRoomId);
    },
    //获取解码器通道配置信息
    httpinfo3() {
      let _this = this;
      _this.$http
        .post("/remoteTrial/getDecodeChannelInfosByPage", {
          trialRoomId: this.trialRoomId, //法庭id
        ...
        .catch(err => {
          console.log(err, "getDecodeChannelInfosByPage");
          _this.$msge("服务器异常，请稍后重试");
        });
    },
    //分页信息
    ...
    add3() {
      //调用子组件方法
      this.$refs.infoAlertsthree3.receiveID(this.trialRoomId);
    ...
    delinfo3(index, row) {
      ...
      _this
        ...
        .then(() => {
          _this.$http
            .post("/remoteTrial/deleteDecodeChannelInfo", {
              signalSourceId: row.signalSourceId //信号源id
            })
            .then(data => {
              if (data.code === 200) {
                _this.$msgs("删除成功!");
                ...
                if (
                  index == 0 &&
                  Math.ceil(Number(_this.total3) / 10) ==
                    Number(_this.pageIndex3) &&
                  _this.pageIndex3 != 1 &&
                  Number(_this.total3) % 10 == 1
                ) {
