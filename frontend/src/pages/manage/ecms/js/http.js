...
import showToast from '../component/partLoading/showToast'
...
class MyAjax{
    ...
    //post请求
     post(url,data,resDate,isLoadmsg) {
         if(isLoadmsg)showToast.open(isLoadmsg)
    ...
    syncRequest(url,param) {
        ...
        if(FoundationTools.getCookie('cmsuserinfo')){
            ...
            param.userId=cmsuserinfo.userid;//添加用户id
            //param.courtCode=FoundationTools.getCookie('hx-lawcourtcode');//添加当前法院
            param.courtCode=Number(sessionStorage.getItem('hx-lawcourtcode'))
        }
        const data = {
            method:'',
            requestSource:'',
