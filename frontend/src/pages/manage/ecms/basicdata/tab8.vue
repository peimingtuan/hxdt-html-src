    ...
    <div class="tab8">
        ...
        <div class="table-style">
            <el-table
                ...
                <el-table-column prop="applyProgramName" label="适用程序名称" show-overflow-tooltip>
                </el-table-column>
                <el-table-column prop="applyProgramCode" label="适用程序代码" show-overflow-tooltip>
                </el-table-column>
...
<script>
    import addbasicAlert from "./tab8/addbasicAlert8" //添加弹框
    export default {
        name: "tab8",
        components:{
            addbasicAlert,
        ...
        methods: {
            getApplyProgramConfigs(){
                ...
                that.$http.post('/applyProgramConfig/getApplyProgramConfigs', {
                    pageIndex,
                    pageSize
                }).then(data => {
            ...
            addlawsure(formLabelAlign){
                if(!formLabelAlign.applyProgramName){
                    this.$msgw("请输入适用程序名称!")
                    return
                }
                if(!formLabelAlign.applyProgramCode){
                    this.$msgw("请输入适用程序代码!")
                    return
                }
                let r=/^\+?[0-9][0-9]*$/;
                if(!r.test(formLabelAlign.applyProgramCode)){
                    this.$msgw("适用程序代码必须为非负整数!")
                    formLabelAlign.applyProgramCode='';
                    return
                }
                const that=this;
                that.$showToast('正在保存...')
                that.$http.post('/applyProgramConfig/saveOrUpdateApplyProgramConfig', {
                    applyProgram:formLabelAlign
                }).then(data => {
                    that.$hideToast()
                    if (data.code === 200) {
                        that.ismain=false;
                        that.$msgs('保存成功!');
                        that.getApplyProgramConfigs()
                    } else {
                        that.$msgw(data.message);
                    }
                }).catch(err => {
            ...
            delinfo(index,row){
                ...
                _this.$confirm('确认删除', '提示', {
                    ...
                    _this.$http.post('/applyProgramConfig/deleteApplyProgramConfig',{
                        applyProgram:{
                            applyProgramId:row.applyProgramId
                        }
                    }).then(data =>{
                        if(data.code===200){
                            _this.$msgs('删除成功!');
