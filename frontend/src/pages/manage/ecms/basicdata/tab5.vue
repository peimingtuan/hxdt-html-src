    ...
    <div class="tab5">
        ...
        <div class="table-style">
            <el-table
                ...
                <el-table-column prop="name" label="开庭方式名称" show-overflow-tooltip>
                </el-table-column>
                <el-table-column prop="code" label="开庭方式代码" show-overflow-tooltip>
                </el-table-column>
...
<script>
    import addbasicAlert from "./tab5/addbasicAlert5" //添加弹框
    export default {
        name: "tab5",
        components:{
            addbasicAlert,
        ...
        methods: {
            getCourtWayConfigList(){
                let {pageIndex,pageSize}=this;
                const that=this;
                that.$showLoading('rgba')
                that.$http.post('/courtWayConfig/getCourtWayConfigList', {
                    pageIndex,
                    pageSize
                }).then(data => {
            ...
            addlawsure(formLabelAlign){
                ...
                if(!formLabelAlign.code){
                    this.$msgw("请输入开庭方式代码!")
                    return
                }
                let r=/^\+?[0-9][0-9]*$/;
                if(!r.test(formLabelAlign.code)){
                    this.$msgw("开庭方式代码必须为非负整数!")
                    formLabelAlign.code='';
                    return
                }
                const that=this;
                that.$showToast("正在保存...")
                that.$http.post('/courtWayConfig/saveOrUpdateCourtWayConfig', {
                    courtWay:formLabelAlign
                }).then(data => {
                    that.$hideToast()
                    if (data.code === 200) {
                        that.ismain=false;
                        that.$msgs('保存成功!');
                        that.getCourtWayConfigList()
                    } else {
                        that.$msgw(data.message);
                    }
                }).catch(err => {
            ...
            delinfo(index,row){
                ...
                _this.$confirm('确认删除', '提示', {
                    ...
                    _this.$http.post('/courtWayConfig/deleteCourtWayConfig',{
                        courtWay:{
                            courtWayId:row.courtWayId
                        }
                    }).then(data =>{
                        if(data.code===200){
                            _this.$msgs('删除成功!');
