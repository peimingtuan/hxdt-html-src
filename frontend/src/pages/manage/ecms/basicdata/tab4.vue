    ...
    <div class="tab4">
        ...
        <div class="table-style">
            <el-table
                ...
                <el-table-column prop="peopleAssessorName" label="人民陪审员名称" show-overflow-tooltip>
                </el-table-column>
...
<script>
    import addbasicAlert from "./tab4/addbasicAlert4" //添加弹框
    export default {
        name: "tab4",
        components:{
            addbasicAlert,
        ...
        methods: {
            getPeopleAssessorConfigList(){
                ...
                that.$http.post('/peopleAssessorConfig/getPeopleAssessorConfigList', {
                    pageIndex,
                    pageSize
                }).then(data => {
            ...
            addlawsure(formLabelAlign){
                if(!formLabelAlign.peopleAssessorName){
                    this.$msgw("请输入人民陪审员名称!")
                    return
                }
                this.$showToast('正在保存...')
                const that=this;
                that.$http.post('/peopleAssessorConfig/saveOrUpdatePeopleAssessorConfig', {
                    peopleAssessor:formLabelAlign
                }).then(data => {
                    that.$hideToast()
                    if (data.code === 200) {
                        that.ismain=false;
                        that.$msgs('保存成功!');
                        that.getPeopleAssessorConfigList()
                    } else {
                        that.$msgw(data.message);
                    }
                }).catch(err => {
            ...
            delinfo(index,row){
                ...
                _this.$confirm('确认删除', '提示', {
                    ...
                    _this.$http.post('/peopleAssessorConfig/deletePeopleAssessorConfig',{
                        peopleAssessor:{
                            peopleAssessorId:row.peopleAssessorId
                        }
                    }).then(data =>{
                        if(data.code===200){
                            _this.$msgs('删除成功!');
                            ...
                            _this.getPeopleAssessorConfigList()
                        }else{
