<template>
    <div class="basictabpane3">
        ...
        <!--添加弹框-->
        <addotherAlert3 :formLabelAlign="formLabelAlign" :alertshow="ismain" @selectshow="ismain=false" @qure="addlawsure" @cancel="ismain=false"/>
    </div>
</template>

<script>
    import addotherAlert3 from "./addotherAlert3" //添加弹框
    export default {
        name: "basictabpane3",
        components: {
            addotherAlert3
        ...
        methods: {
            ...
            getsignalTypelist(){
                ...
                let _this=this;
                return _this.$http.post('/otherConfig/searchSignalTypeConfig', {
                            keyWord,//搜索关键词
                            pageIndex,//当前页码
                            pageSize //每页大小
                        }).then(data =>{
            ...
            addlawsure(formLabelAlign){
                let _this=this;
                let url='/otherConfig/updateSignalTypeInfo';//更新
                if(formLabelAlign.signalTypeId=='')url='/otherConfig/addSignalTypeInfo';//新增
            ...
            delinfo(index,row){
                ...
                _this.$confirm('确认删除', '提示', {
                    ...
                    _this.$http.post('/otherConfig/delSignalTypeInfo', {
                        signalTypeId:row.signalTypeId//要删除的信号类型id
                    }).then(data =>{
                        if(data.code===200){
