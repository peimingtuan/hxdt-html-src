    ...
    export default {
        ...
        data() {
            return {
                ...
                //底部查询功能
                lawname:this.$route.query.courtName || '',//法院名称
                searchname:'',//检索词
                datestartval:'',//开始日期
                dateendval:'',//结束日期
                //开始日期
                pickerOptions0: {
                    disabledDate: (time) => {
                        if (this.dateendval) {
                            ...
                            return time.getTime() > this.dateendval;
                        } else {
        ...
        mounted() {
            ...
            if(this.$route.query.causeName && this.$route.query.causeId){
                ...
                this.$store.commit('sethttpurl','/getCaseListWebPage')
        ...
        methods: {
            ...
            loadNode(node, resolve) {
                if (node.level === 0) {
                    return resolve([{causeId:this.$route.query.causeId,causeName:this.$route.query.causeName || '认为侵犯土地承包经营自主权',causeTypeId:false,parentId2:[] }]);
                }
                // if (node.level > 1) return resolve([]);
                let that=this;
                that.$http.post('/getTreeCause', {
            ...
            leftselect(){
                ...
                _this.$store.commit('sethttpurl','/getCaseListWebPage')
            ...
            selectlaw(){
                let _this=this;
                if(!_this.lawname && !_this.searchname && !_this.datestartval && !_this.dateendval){
                    _this.$msgw('请选择查询条件');
                }else{
                    if(!_this.lawname){
                        _this.$msgw('请输入法院名称');
                    ...
                    console.log(typeof _this.datestartval)
                    if(_this.datestartval && typeof _this.datestartval !='string')_this.datestartval=FoundationTools.getFormatDate2(_this.datestartval);
                    //else _this.datestartval=''
                    if(_this.datestartval==null)_this.datestartval=''
                    if(_this.dateendval && typeof _this.dateendval !='string')_this.dateendval=FoundationTools.getFormatDate2(_this.dateendval);
                    //else _this.dateendval=''
                    if(_this.dateendval==null)_this.dateendval=''
                    _this.$store.commit('setkeyword',_this.searchname)
                    _this.$store.commit('setStartdate',_this.datestartval)
                    _this.$store.commit('setEnddate',_this.dateendval)
                    _this.$store.commit('setCourtName',_this.lawname)
                    _this.$store.commit('sethttpurl','/getCauseListByKeyword')
                    _this.$store.commit('setcaseidlist',_this.checklist.join(','))
                    _this.changepageindex(1)//改变页码为当前第一页
