<template>
    <!--弹框-->
    <transition name="slide-fade">
        <div class="alert-window2 eltreeAlert2" v-if="showAlert">
            <div class="menu">
                <img class="alert-close" @click="showAlert=false" src="../images/alert-close.png" />
                <div class="title">
                    选择案由
                </div>
                <div class="search">
                    <el-input
                            placeholder="输入关键字进行过滤"
                            v-model.trim="filterText"
                            clearable>
                    </el-input>
                    <el-button type="primary" icon="el-icon-search" @click="search(filterText)">
                        搜索
                    </el-button>
                </div>
                <!--树形菜单-->
                <el-tree
                        :props="defaultProps"
                        ref="tree2"
                        :data="causeList"
                        :default-expanded-keys="causeCodes"
                        show-checkbox
                        node-key="causeCode"
                        check-strictly="false"
                        :filter-node-method="filterNode"
                        @check-change="handleCheckChange"
                        @node-click="handleNodeClick">
                </el-tree>
                <alert-btn
                        ref="alertBtn"
                        @qure="sureChoice"
                        @cancel="showAlert=false">
                </alert-btn>
            </div>
        </div>
    </transition>
</template>

<script>
    import alertBtn from '../../ecms/component/alertBtn'
    export default {
        name: "eltreeAlert2",
        components:{
            alertBtn
        },
        data() {
            return {
                showAlert:false,
                //案由树配置
                defaultProps: {
                    children: 'childrens',
                    label: 'causeName',
                },
                causeList:[],
                checklist:[],
                causeCodes:[],
                filterText:'',
            }
        },
        created() {

        },
        mounted() {

        },
        destroyed() {

        },
        watch: {
            filterText(val) {
                if(!val){
                    this.$refs.tree2.filter(val);
                }
            }
        },
        methods: {
            search(val){
                if(val){
                    this.$refs.tree2.filter(val);
                }else{
                    this.$msgw('请输入关键字')
                }
            },
            //展示弹框
           async show(valueCodes,values){
               //this.causeList=[];
               //this.checklist=[];
               //this.causeCodes=[];
               this.showAlert=true;
               this.filterText='';
               if(this.causeList.length===0){
                   await this.getAllCauseStr()
               }
               //if(valueCodes.length>0){
                   let [...arr]=values,
                       [...arr1]=valueCodes;
                   this.checklist=arr;
                   this.causeCodes=arr1;
                   this.$nextTick(()=>{
                       this.setCheckedKeys(arr1)
                   })
               //}
            },
            //选中当前节点
            handleNodeClick(data) {
                // this.$parent.formLabelAlign.causeCode=data.causeCode;
                // this.$parent.formLabelAlign.causeName=data.causeName;
                // this.showAlert=false;
            },
            filterNode(value, data,node) {
                if (!value) return true;
                return data.causeName.indexOf(value) !== -1;
            },
            //设置被选中节点
            setCheckedKeys(valueCodes) {
                this.$refs.tree2.setCheckedKeys(valueCodes);
            },
            handleCheckChange(data,node,index){
                if(node){
                    this.checklist.push(data.causeName)
                    this.causeCodes.push(data.causeCode)
                    //数组先去重
                    this.checklist=[...new Set(this.checklist)]
                    this.causeCodes=[...new Set(this.causeCodes)]
                }else{
                    let index = this.checklist.indexOf(data.causeName);
                    this.checklist.splice(index, 1);
                    this.causeCodes.splice(index, 1);
                }
            },
            getAllCauseStr(){
                this.$showToast('加载中...')
                const that=this;
                return that.$http.post('/baseInfo/getAllCauseStr', {

                }).then(data => {
                    that.$hideToast()
                    if (data.code === 200) {
                        that.causeList=data.data.causeList;
                    } else {
                        that.$msgw(data.message);
                    }
                }).catch(err => {
                    that.$hideToast()
                    that.$msge('服务器异常，请稍后重试');
                })
            },
            //确定
            sureChoice(){
                if(this.checklist.length===0){
                    this.$msgw('请选择案由!')
                    return
                }
                this.showAlert=false;
                this.$parent.values=this.checklist;
                this.$parent.valueCodes=this.causeCodes;
            },
        }
    }
</script>

<style lang="less">
    @import url('../css/common.less');
    .eltreeAlert2{
        .menu {
            width:8rem;
            padding:0 .8rem;
            box-shadow: -5px 5px 5px 0 rgba(38,78,119,.2),5px -3px 3px 0 rgba(38,78,119,.2);
            background:#fff;
            position:absolute;
            left:50%;
            top:1rem;
            margin-left:-3rem;
            border-radius:2px;
            cursor: pointer;
            /*关闭弹框*/
            img.alert-close{
                width:.18rem;
                position:absolute;
                top: .25rem;
                right: .35rem;
                cursor: pointer;
                z-index:102;
            }
            .title{
                position:absolute;
                left:50%;
                top:0;
                width:2rem;
                margin-left:-1rem;
                line-height:.6rem;
                text-align: center;
            }
            .search{
                margin:.7rem 0 .2rem;
                .fb();
                .el-button {
                    margin-left:.1rem;
                    position: relative;
                    top: -0.01rem;
                    width:1.5rem;
                    .el-icon-search {
                        .f18();
                        color: #fff;
                    }

                    span {
                        .f18();
                        color: #fff;
                    }
                }
            }
            .el-tree{
                /*margin:.6rem 0;*/
                min-height:5rem;
                max-height:6rem;
                overflow: auto;
                background: #eceff0;
                .el-tree-node__content {
                    height: .58rem;
                    .el-tree-node__label {
                        width: 4.7rem;
                        overflow: hidden;
                        text-overflow: ellipsis;
                        white-space: nowrap;
                        .f22();
                    }
                    /*icon*/
                    .el-icon-caret-right:before {
                        font-size: .2rem;
                    }

                    .el-checkbox__inner:before {
                        top: .07rem;
                        height: .02rem;
                    }
                }
                /*span*/
                .el-tree-node__label {
                    .f20();
                    color: #303133;
                }
                /*checkbox*/
                .el-checkbox__inner {
                    width: .18rem;
                    height: .18rem;
                }
                /*选中*/
                .el-checkbox__inner:after {
                    top: .02rem;
                    left: .06rem;
                    width: .04rem;
                    height: .08rem;
                }
            }
        }
    }
</style>