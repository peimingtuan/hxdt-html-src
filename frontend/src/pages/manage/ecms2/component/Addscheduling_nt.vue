            ...
            <div class='second-alert' v-show='isAddscheduling_nt'>
                <div class="main-twice">
                    ...
                    <div class="content" id="content">
                        <el-form :label-position="labelPosition" :model="formLabelAlign">
                            ...
                            <el-form-item label="审理形式">
                                <el-select v-model='formLabelAlign.trialFormCode'>
                                    <el-option v-for="item in TrialWaylist" :key="item.code" :label="item.name"
                                               :value="item.code">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="审理模式">
                                <el-select v-model="formLabelAlign.trialMode" placeholder="请选择">
                                    <el-option v-for="item in TrialModeInfosList" :key="item.code" :label="item.name"
                                               :value="item.code">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                        ...
                        <el-form :label-position="labelPosition" :model="formLabelAlign">
                            <!-- 审判长，审判员 -->
                            <el-form-item class="red-label" label="是否独任审理">
                                <el-radio-group v-model="formLabelAlign.soleTrial" @change="radiochange">
                                    <el-radio :label=true>是</el-radio>
                                    <el-radio :label=false>否</el-radio>
                                </el-radio-group>
                            </el-form-item>
        ...
        data() {
            return {
                ...
                formLabelAlign: {
                    ...
                    publicTrial: true,	//是否公开审理
                    soleTrial: false, //是否独任审理
                    courtTime: "",//庭次
                    courtWay: "",//开庭方式
        ...
        methods: {
            ...
            getRemoteTrialTemplates(courtRoomId){
                ...
                return  _this.$http.post('/getRemoteTrialTemplates',param).then(data =>{
                    ...
                    _this.$hideToast()
                    console.log(err,'/getRemoteTrialTemplates')
                })
            },
            //选择
            ...
            see(tableData,row) {
                if (row) {
                    ...
                    this.formLabelAlign.planStartDate = row.factStartDate
                    this.formLabelAlign.planStartTime = row.factStartTime
                    this.formLabelAlign.planEndTime = row.factEndTime
                    this.value1='';
                    this.value2 = [];
                    this.value3 = [];
                    this.value4='';
                    this.value5='';
                    if (this.formLabelAlign.trialMemberList.length > 0) {
                        this.formLabelAlign.trialMemberList.map(item => {
                            if (item.trialMemberTypeCode == 1) {
                                this.value1 = item.userId;
                            } else if (item.trialMemberTypeCode == 2) {
                                this.value2.push(item.userId);
                                //this.value2=item.userId.split(';')//审判员
                            } else if (item.trialMemberTypeCode == 4) {
                                this.value3.push(item.userId);
                                //this.value3=item.userId.split(';')//人民审判员
                            } else if (item.trialMemberTypeCode == 6) {
                                this.value4 = item.userId;
                            }
                        })
                    }
                    ...
                    this.formLabelAlign = {
                        ...
                        caseNumber: '',//案号
                        trialMode:1,	//审理模式
                        trialFormCode:1,//审理形式
                        planStartDate:FoundationTools.getFormatDate2(new Date()) || '',	//开庭日期
                        planStartTime: "",	//开庭时间
                        planEndTime: "",	//结束时间
                        courtRoomId: "",//开庭地点id
                        courtRoomName: "",//	开庭地点（法庭名称）
                    ...
                    if(tableData){
                        if(tableData.length===0)this.formLabelAlign.courtTime=1;
                        else{
                            let arr=tableData.map(item=>{
                                return item.courtTime
                            })
                            this.formLabelAlign.courtTime=arr.reduce((num1,num2)=>{
            ...
            sure() {
                ...
                if(!this.formLabelAlign.planStartDate){
                    this.$msgw("请选择开庭日期！")
                    return false
                }
                if(!this.formLabelAlign.planStartTime){
                    this.$msgw("请选择开庭开始时间！")
                    return false
                }
                if(!this.formLabelAlign.planEndTime){
                    this.$msgw("请选择开庭结束时间！")
                    return false
                }
                if(!this.formLabelAlign.courtRoomName){
                    this.$msgw("请输入法庭！")
                    return false
                }
