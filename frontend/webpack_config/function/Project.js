...
// 项目根目录，对应 hxdt-html-src/frontend
const PROJECT_ROOT = path.join(__dirname, '../../')
// 待发布的目录，对应 hxdt-html/frontendDist
const PROJECT_BUILD_ROOT = path.join(PROJECT_ROOT, '../../', 'hxdt-html/frontendDist/')
...
module.exports = class Project {
  ...
  pps：欢迎加入华夏电通前端组`)
    ...
    switch (mode) {
      case 1:
        return `Copyright © since 2020 华夏电通 版权所有 Recent edit by ${user.name}/${user.email} on ${new Date().toLocaleString()}`
      default:
        return `Copyright © 2020 华夏电通 版权所有
Project name:${this.group}_${this.name}
Version:${user.name}/peimingtuan
